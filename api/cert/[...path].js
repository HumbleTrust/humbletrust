// HTCERT — Launch Certificate metadata & image
// Routes:
//   GET /api/cert/:mint/metadata.json  → Metaplex-compatible NFT metadata
//   GET /api/cert/:mint/image.svg      → Zodiac badge SVG (same design as ZodiacBadgeCard)

const { getClient }  = require('../_lib/db');
const { setCors }    = require('../_lib/validate');
const { getZodiac, getAuraColor } = require('../badges/_zodiac');

const SOLANA_ADDR_RE = /^[1-9A-HJ-NP-Za-km-z]{32,44}$/;

// ─── SVG helpers (matches ZodiacBadgeCard React component design) ─────────────
...
function generateCertSVG(zodiac, element, aura, edition, tokenName, tokenSymbol) {
  const W = 440, H = 660;
  const [g1, g2] = ELEMENT_GRADIENT[element] ?? ['#1a0a2e', '#05070F'];
  const dk = darken(aura);
  const [r, g, b] = hexToRgb(aura);
  const edStr = String(edition).padStart(4, '0');
  ...
  const conStars = con.stars.map(([x, y, sr]) =>
    `<circle cx="${x*2}" cy="${y*2}" r="${sr*1.8}" fill="white" opacity="0.55"/>`).join('');
  ...
  const sc = 2.6, sx = (W - 90 * sc) / 2, sy = 90;

  // Truncate long token names for display
  const displayName = tokenName && tokenName.length > 18
    ? tokenName.slice(0, 16) + '..'
    : (tokenName || '');
  const displaySymbol = tokenSymbol ? `$${tokenSymbol.slice(0, 8).toUpperCase()}` : '';
...
<!-- Header: HTCERT label + token symbol -->
<text x="26" y="36" font-family="monospace,Courier New" font-size="11" fill="rgba(255,255,255,0.28)" letter-spacing="3">HTCERT</text>
<text x="${W-26}" y="36" font-family="monospace,Courier New" font-size="11" fill="${aura}" letter-spacing="2" text-anchor="end" opacity="0.8">${displaySymbol}</text>
<!-- Footer: cert # + LAUNCH CERTIFICATE -->
<text x="26" y="${H-22}" font-family="monospace,Courier New" font-size="11" fill="rgba(255,255,255,0.22)" letter-spacing="2">#${edStr}</text>
<text x="${W-26}" y="${H-22}" font-family="monospace,Courier New" font-size="11" fill="rgba(255,255,255,0.22)" letter-spacing="2" text-anchor="end">LAUNCH CERT</text>
<!-- Shield -->
<g transform="translate(${sx.toFixed(1)},${sy}) scale(${sc})">
  ...
  <!-- HT logo watermark inside shield -->
  <g opacity="0.6">
    <!-- HT logo: simplified geometric mark matching brand -->
    <rect x="30" y="20" width="30" height="26" rx="3" fill="none" stroke="${aura}" stroke-width="0.8" opacity="0.3"/>
    <text x="45" y="38" font-family="monospace" font-size="14" fill="${aura}" text-anchor="middle" font-weight="bold" opacity="0.7">HT</text>
  </g>
  <!-- Zodiac glyph on top -->
  <g opacity="0.85">
  ...
  <!-- Verified checkmark badge (top-right of shield) -->
...
<!-- Element sigil -->
<g transform="translate(${(W / 2 - 44).toFixed(0)},${(sy + 90 * sc + 20).toFixed(0)})">
  <svg viewBox="0 0 80 80" width="88" height="88">
    ${elementSigil(element, aura)}
  </svg>
</g>
<!-- Token name -->
<text x="${W/2}" y="${(sy + 90*sc + 115).toFixed(0)}" font-family="monospace,Courier New" font-size="15" fill="${aura}" letter-spacing="3" text-anchor="middle" opacity="0.9">${displayName.toUpperCase()}</text>
<!-- Zodiac name -->
<text x="${W/2}" y="${(sy + 90*sc + 140).toFixed(0)}" font-family="monospace,Courier New" font-size="18" fill="white" letter-spacing="6" text-anchor="middle" opacity="0.92">${zodiac.toUpperCase()}</text>
<line x1="${W/2 - 45}" y1="${(sy + 90*sc + 155).toFixed(0)}" x2="${W/2 + 45}" y2="${(sy + 90*sc + 155).toFixed(0)}" stroke="${aura}" stroke-width="1" opacity="0.6"/>
<text x="${W/2}" y="${(sy + 90*sc + 178).toFixed(0)}" font-family="monospace,Courier New" font-size="13" fill="${aura}" letter-spacing="5" text-anchor="middle" opacity="0.9">${element.toUpperCase()}</text>
</svg>`;
}
...
async function getTokenData(mint) {
  ...
  const { data } = await db
    .from('tokens')
    .select('name,symbol,creator,created_at')
    .eq('mint', mint)
    .maybeSingle();
  return data;
}

// Edition = rank of this token among all launches (chronological order)
async function getTokenEdition(mint, createdAt) {
  const db = getClient();
...
async function handleMetadata(req, res, mint) {
  let zodiac = 'Capricorn', element = 'Earth', aura = '#9945FF';
  ...
  const auraHex  = aura.replace('#', '');
  const imageUrl = `${origin}/api/cert/${mint}/image.svg?z=${encodeURIComponent(zodiac)}&el=${encodeURIComponent(element)}&a=${auraHex}&ed=${edition}&nm=${encodeURIComponent(tokenName)}&sy=${encodeURIComponent(tokenSymbol)}`;
  const edStr    = String(edition).padStart(4, '0');
  const launchDate = new Date(createdAt).toISOString().slice(0, 10);

  res.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
  return res.json({
    name:         `${tokenName} Launch Certificate`,
    symbol:       'HTCERT',
    description:  `Official HumbleTrust Launch Certificate for ${tokenName} ($${tokenSymbol}). Minted on ${launchDate} · ${zodiac} · ${element} · Edition #${edStr}. Soulbound — non-transferable proof of honest launch.`,
    image:        imageUrl,
    ...
    attributes: [
      ...
      { trait_type: 'Launch Date',   value: launchDate   },
      { trait_type: 'Zodiac',        value: zodiac        },
      { trait_type: 'Element',       value: element       },
      { trait_type: 'Aura',          value: `#${auraHex}` },
      { trait_type: 'Edition',       value: edition       },
      { trait_type: 'Certificate',   value: 'HTCERT'      },
    ...
    collection: { name: 'HumbleTrust Launch Certificates', family: 'HumbleTrust' },
  });
}

async function handleImage(req, res, mint) {
  // Prefer query params (pre-computed from metadata handler); fallback to DB lookup
  let zodiac = req.query.z, element = req.query.el;
  let aura   = req.query.a ? `#${req.query.a.replace('#', '')}` : null;
  let edition = parseInt(req.query.ed) || 1;
  let tokenName   = req.query.nm ? decodeURIComponent(req.query.nm) : '';
  let tokenSymbol = req.query.sy ? decodeURIComponent(req.query.sy) : '';

  const VALID_ZODIAC_SET = new Set([
    'Aries','Taurus','Gemini','Cancer','Leo','Virgo',
    'Libra','Scorpio','Sagittarius','Capricorn','Aquarius','Pisces',
  ]);
  const VALID_ELEMENT_SET = new Set(['Fire','Water','Earth','Air']);

  const needsLookup = !zodiac || !element || !aura
    || !VALID_ZODIAC_SET.has(zodiac)
  ...
  if (needsLookup && mint && mint !== 'generic') {
    try {
      const token = await getTokenData(mint);
      if (token) {
        const z  = getZodiac(new Date(token.created_at));
        zodiac   = z.name;
        element  = z.element;
        aura     = getAuraColor(token.creator || '');
  ...
  const svg = generateCertSVG(zodiac, element, aura, edition, tokenName, tokenSymbol);
...
module.exports = async (req, res) => {
  ...
  if (filepart === 'image.svg')     return handleImage(req, res, mint);
  return res.status(404).json({ error: 'Not found. Use /metadata.json or /image.svg' });
};
