...
export const Discover = ({ openToken }: { openToken: (mint: string) => void }) => {
  ...
  const localByMint = useMemo(() => new Map(localTokens.map(t => [t.mint, t])), [localTokens]);

  const load = async () => {
    setBusy(true);
    try {
      const result = await getTokens(150);
      // Merge: API data wins for indexed fields, local fills in browser-only logo data until Supabase has it.
      const mergedApi = result.tokens.map(t => {
        const local = localByMint.get(t.mint);
        return {
          ...t,
          logo_url: t.logo_url || local?.logo_url || null,
          certificate_mint: t.certificate_mint || local?.certificate_mint || null,
        };
      });
      const apiMints = new Set(mergedApi.map(t => t.mint));
      const localOnly = localTokens.filter(t => !apiMints.has(t.mint));
      setTokens([...mergedApi, ...localOnly]);
      setApiOk(true);
    } catch {
          ...
          return (
            <div key={t.mint} className="tok-card-real token-card-click" onClick={() => openToken(t.mint)}>
              <div className="tok-card-top">
                <HexLogo src={t.logo_url || undefined} label={t.symbol || t.mint.slice(0, 3)} size={56} variant={Number(t.trust_score) >= 85 ? "gradient" : "green"} />
